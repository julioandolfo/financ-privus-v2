<x-layouts.app title="Editar Pedido #{{ $pedido->numero_pedido }}">

    <div class="max-w-5xl mx-auto">

        {{-- Page header --}}
        <div class="flex items-center gap-3 mb-6">
            <a href="{{ route('pedidos.show', $pedido) }}" class="text-surface-400 hover:text-surface-600 dark:hover:text-surface-300 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5 3 12m0 0 7.5-7.5M3 12h18" />
                </svg>
            </a>
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Editar Pedido</h1>
                <p class="text-sm text-surface-500 mt-0.5">{{ $pedido->numero_pedido }}</p>
            </div>
        </div>
            ...
            x-data="{
                itens: {{ Js::from(
                    $pedido->itens->map(fn($item) => [
                        ...
                        'codigo_produto_origem' => $item->codigo_produto_origem ?? '',
        ...
        >
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <x-ui.select name="cliente_id" label="Cliente" :error="$errors->first('cliente_id')">
                        ...
                        <option value="{{ $c->id }}" @selected(old('cliente_id', $pedido->cliente_id) == $c->id)>{{ $c->nome_razao_social }}</option>
                        @endforeach
                    </x-ui.select>

                    <x-ui.select name="origem" label="Origem" required :error="$errors->first('origem')">
                        <option value="manual"      @selected(old('origem', $pedido->origem) === 'manual')>Manual</option>
                        <option value="woocommerce" @selected(old('origem', $pedido->origem) === 'woocommerce')>WooCommerce</option>
                        <option value="marketplace" @selected(old('origem', $pedido->origem) === 'marketplace')>Marketplace</option>
                    </x-ui.select>
                    ...
                    <x-ui.input
                        ...
                        value="{{ old('origem_id', $pedido->origem_id) }}"
                    ...
                    <x-ui.select name="status" label="Status" required :error="$errors->first('status')">
                        ...
                        <option value="processando" @selected(old('status', $pedido->status) === 'processando')>Processando</option>
                        <option value="concluido"   @selected(old('status', $pedido->status) === 'concluido')>Concluído</option>
                        <option value="cancelado"   @selected(old('status', $pedido->status) === 'cancelado')>Cancelado</option>
                        <option value="reembolsado" @selected(old('status', $pedido->status) === 'reembolsado')>Reembolsado</option>
                    </x-ui.select>
                    ...
                    <x-ui.input
                        ...
                        value="{{ old('status_origem', $pedido->status_origem) }}"
            ...
            <x-ui.card class="mb-6">
                ...
                >{{ old('observacoes', $pedido->observacoes) }}</x-ui.textarea>
            </x-ui.card>

            <div class="flex items-center justify-end gap-3">
                <x-ui.button href="{{ route('pedidos.show', $pedido) }}" variant="ghost">Cancelar</x-ui.button>
