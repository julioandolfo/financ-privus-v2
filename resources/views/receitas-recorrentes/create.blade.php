<x-layouts.app title="Nova Receita Recorrente">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Nova Receita Recorrente</h1>
                <p class="text-sm text-surface-500 mt-0.5">Configure uma receita que se repete automaticamente</p>
            </div>
        </div>
        @if($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('receitas-recorrentes.store') }}" x-data="{ frequencia: '{{ old('frequencia', 'mensal') }}' }">
            @csrf
            @include('receitas-recorrentes._form')
