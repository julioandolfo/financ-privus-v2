<x-layouts.app title="Receitas Recorrentes">

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Receitas Recorrentes</h1>
            <p class="text-sm text-surface-500 mt-0.5">Receitas fixas geradas automaticamente</p>
        </div>
        <x-ui.button href="{{ route('receitas-recorrentes.create') }}">
            ...
            <table class="min-w-full divide-y divide-surface-100 dark:divide-surface-700">
                ...
                <tbody class="divide-y divide-surface-100 dark:divide-surface-700">
                    ...
                    <tr class="hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors {{ !$r->ativo ? 'opacity-50' : '' }}">
                        ...
                        <td class="px-5 py-4 text-sm text-surface-500">{{ $r->cliente?->nome_razao_social ?? '—' }}</td>
                        <td class="px-5 py-4">
                            <x-ui.badge variant="secondary">{{ $r->frequencia_label }}</x-ui.badge>
                        </td>
                        <td class="px-5 py-4 text-sm font-semibold text-right text-green-600 dark:text-green-400 whitespace-nowrap">
                        ...
                        <td class="px-5 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-1">
                                <form method="POST" action="{{ route('receitas-recorrentes.gerar', $r) }}">
                                    @csrf
                                ...
                                <a href="{{ route('receitas-recorrentes.edit', $r) }}" class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium text-surface-600 hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors">Editar</a>
                                <form method="POST" action="{{ route('receitas-recorrentes.toggle', $r) }}">
                                    @csrf
                    ...
                    <tr><td colspan="7" class="px-5 py-12 text-center text-sm text-surface-400">
                        Nenhuma receita recorrente. <a href="{{ route('receitas-recorrentes.create') }}" class="text-primary-600 hover:underline">Criar primeira</a>
                    </td></tr>
                    @endforelse
                </tbody>
            </table>
