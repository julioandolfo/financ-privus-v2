<x-layouts.app title="Novo Perfil de Consolidação">
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Novo Perfil de Consolidação</h1>
                <p class="text-sm text-surface-500 mt-0.5">Defina uma visão personalizada de dados financeiros</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('perfis-consolidacao.store') }}">
            @csrf
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="space-y-4">
                    <x-ui.input
                        ...
                        maxlength="100"
                        value="{{ old('nome') }}"
                        placeholder="Ex.: Resultado Operacional Q1"
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-ui.select name="periodo" label="Período" required :error="$errors->first('periodo')">
                        <option value="mes_atual" @selected(old('periodo', 'mes_atual') === 'mes_atual')>Mês Atual</option>
                        <option value="trimestre" @selected(old('periodo') === 'trimestre')>Trimestre Atual</option>
                        <option value="ano"        @selected(old('periodo') === 'ano')>Ano Atual</option>
                    </x-ui.select>

                    <x-ui.select name="tipo" label="Tipo de Dados" required :error="$errors->first('tipo')">
                        <option value="ambos"    @selected(old('tipo', 'ambos') === 'ambos')>Receitas e Despesas</option>
                        <option value="receitas" @selected(old('tipo') === 'receitas')>Somente Receitas</option>
                        <option value="despesas" @selected(old('tipo') === 'despesas')>Somente Despesas</option>
                    </x-ui.select>
                </div>
            </x-ui.card>
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <x-ui.button type="submit">Criar Perfil</x-ui.button>
            </div>
        </form>
    </div>
