<x-layouts.app title="Contas a Receber">

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Contas a Receber</h1>
            <p class="text-sm text-surface-500 mt-0.5">Gerencie seus recebíveis</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('contas-receber.deletados') }}"
            ...
            <x-ui.button href="{{ route('contas-receber.create') }}">
    ...
    <x-ui.card class="mb-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            ...
            <x-ui.button href="{{ route('contas-receber.index') }}" variant="ghost">Limpar</x-ui.button>
            @endif
        </form>
    </x-ui.card>
    ...
    <x-ui.card :padding="false" x-data="{ selected: [], all: false, acaoMassa: 'baixa' }">
        ...
        <div x-show="selected.length > 0" x-cloak class="flex flex-wrap items-center gap-3 px-5 py-3 bg-green-50 dark:bg-green-900/20 border-b border-green-100 dark:border-green-800">
            <span class="text-sm font-medium text-green-700 dark:text-green-300" x-text="selected.length + ' selecionado(s)'"></span>
            ...
            {{-- Baixa em massa --}}
            <form x-show="acaoMassa === 'baixa'" method="POST" action="{{ route('contas-receber.baixa-massa') }}" class="flex items-center gap-2">
                @csrf
                <template x-for="id in selected" :key="id">
                    <input type="hidden" name="ids[]" :value="id">
                </template>
                <x-ui.input type="date" name="data_recebimento" required class="py-1.5 text-xs w-36" />
                <x-ui.button type="submit" size="sm" variant="primary">Baixar Selecionados</x-ui.button>
            </form>

            {{-- Atualizar categoria em massa --}}
            <form x-show="acaoMassa === 'categoria'" method="POST" action="{{ route('contas-receber.atualizar-categoria-massa') }}" class="flex items-center gap-2">
                @csrf
                <template x-for="id in selected" :key="id">
                    <input type="hidden" name="ids[]" :value="id">
                </template>
            ...
            <form x-show="acaoMassa === 'data'" method="POST" action="{{ route('contas-receber.atualizar-data-massa') }}" class="flex items-center gap-2">
                @csrf
                <template x-for="id in selected" :key="id">
                    <input type="hidden" name="ids[]" :value="id">
                </template>
            ...
            <table class="min-w-full divide-y divide-surface-100 dark:divide-surface-700">
                <thead>
                    <tr class="bg-surface-50 dark:bg-surface-800/50">
                        ...
                        <th class="px-5 py-3 text-right text-xs font-medium text-surface-500 uppercase tracking-wider">Recebido</th>
                ...
                <tbody class="divide-y divide-surface-100 dark:divide-surface-700">
                    ...
                    <tr class="hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                        ...
                        <td class="px-5 py-4 text-sm text-surface-500 text-right whitespace-nowrap">
                            R$ {{ number_format($conta->valor_recebido, 2, ',', '.') }}
                        </td>
                        <td class="px-5 py-4">
                            @php
                                $statusMap = [
                        ...
                        <td class="px-5 py-4 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-1">
                                <a href="{{ route('contas-receber.show', $conta) }}"
                                ...
                                @if(in_array($conta->status, ['pendente','parcial','vencido']))
                                <a href="{{ route('contas-receber.edit', $conta) }}"
                                ...
                                <form method="POST" action="{{ route('contas-receber.destroy', $conta) }}"
                    ...
                    <tr>
                        <td colspan="8" class="px-5 py-12 text-center text-sm text-surface-400">
                            ...
                            <a href="{{ route('contas-receber.create') }}" class="text-primary-600 hover:underline ml-1">Criar nova</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
