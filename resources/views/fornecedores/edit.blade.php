<x-layouts.app title="Editar Fornecedor">
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">{{ $fornecedor->nome_razao_social }}</h1>
                <p class="text-sm text-surface-500 mt-0.5">{{ $fornecedor->cpf_cnpj ?? 'Sem CPF/CNPJ' }}</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('fornecedores.update', $fornecedor) }}">
                ...
                <div class="mb-4">
                    ...
                    <div class="flex gap-4">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="tipo" value="fisica" @checked(old('tipo', $fornecedor->tipo) === 'fisica') class="text-primary-600 focus:ring-primary-500">
                            <span class="text-sm text-surface-700 dark:text-surface-300">Pessoa Física</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="tipo" value="juridica" @checked(old('tipo', $fornecedor->tipo) === 'juridica') class="text-primary-600 focus:ring-primary-500">
            ...
            <x-ui.card class="mb-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-surface-500 mt-0.5">Fornecedor {{ $fornecedor->ativo ? 'ativo' : 'inativo' }} no sistema</p>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="ativo" value="1" @checked(old('ativo', $fornecedor->ativo)) class="sr-only peer">
            ...
            <x-ui.card class="mb-6">
                <x-ui.textarea name="observacoes" label="Observações" rows="3">{{ old('observacoes', $fornecedor->observacoes) }}</x-ui.textarea>
            </x-ui.card>

            <div class="flex items-center justify-between">
                <form method="POST" action="{{ route('fornecedores.destroy', $fornecedor) }}"
                      onsubmit="return confirm('Excluir este fornecedor permanentemente?')">
                    @csrf @method('DELETE')
