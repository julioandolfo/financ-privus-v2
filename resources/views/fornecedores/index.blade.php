<x-layouts.app title="Fornecedores">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Fornecedores</h1>
            <p class="text-sm text-surface-500 mt-0.5">Gerencie sua base de fornecedores</p>
        </div>
        <x-ui.button href="{{ route('fornecedores.create') }}">
    ...
    <x-ui.card class="mb-4">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            <div class="flex-1 min-w-[200px]">
                <x-ui.input name="search" placeholder="Buscar por nome ou CPF/CNPJ..." value="{{ request('search') }}" label="Buscar" />
            </div>
            <x-ui.button type="submit" variant="secondary">Filtrar</x-ui.button>
            @if(request('search'))
            <x-ui.button href="{{ route('fornecedores.index') }}" variant="ghost">Limpar</x-ui.button>
            @endif
        </form>
    </x-ui.card>
            ...
            <table class="min-w-full divide-y divide-surface-100 dark:divide-surface-700">
                ...
                <tbody class="divide-y divide-surface-100 dark:divide-surface-700">
                    ...
                    <tr class="hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                        <td class="px-5 py-4">
                            <div class="text-sm font-medium text-surface-900 dark:text-white">{{ $fornecedor->nome_razao_social }}</div>
                            @if($fornecedor->nome_fantasia)
                            <div class="text-xs text-surface-400">{{ $fornecedor->nome_fantasia }}</div>
                            @endif
                        </td>
                        <td class="px-5 py-4 text-sm text-surface-500">{{ $fornecedor->cpf_cnpj ?? '—' }}</td>
                        <td class="px-5 py-4">
                            @if($fornecedor->email)
                            <div class="text-sm text-surface-700 dark:text-surface-300">{{ $fornecedor->email }}</div>
                            @endif
                            @if($fornecedor->celular)
                            <div class="text-xs text-surface-400">{{ $fornecedor->celular }}</div>
                            @endif
                        </td>
                        <td class="px-5 py-4">
                            <x-ui.badge variant="{{ $fornecedor->tipo === 'juridica' ? 'primary' : 'default' }}">
                                {{ $fornecedor->tipo === 'juridica' ? 'PJ' : 'PF' }}
                            </x-ui.badge>
                        </td>
                        <td class="px-5 py-4">
                            <x-ui.badge variant="{{ $fornecedor->ativo ? 'success' : 'default' }}">
                    ...
                    <tr>
                        <td colspan="6" class="px-5 py-12 text-center text-sm text-surface-400">
                            ...
                            <a href="{{ route('fornecedores.create') }}" class="text-primary-600 hover:underline ml-1">Cadastrar novo</a>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
