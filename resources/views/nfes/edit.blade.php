<x-layouts.app title="Editar NF-e">
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Editar NF-e</h1>
                <p class="text-sm text-surface-500 mt-0.5">Rascunho — {{ $nfe->numero_serie }}</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('nfes.update', $nfe) }}"
              x-data="{
                  valorProdutos: '{{ old('valor_produtos', $nfe->valor_produtos) }}',
                  valorFrete: '{{ old('valor_frete', $nfe->valor_frete) }}',
                  valorDesconto: '{{ old('valor_desconto', $nfe->valor_desconto) }}',
                  get valorTotal() {
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-ui.select name="cliente_id" label="Cliente" :error="$errors->first('cliente_id')">
                        ...
                        <option value="{{ $c->id }}" @selected(old('cliente_id', $nfe->cliente_id) == $c->id)>
                            {{ $c->nome_razao_social }}
                        </option>
                        @endforeach
                    </x-ui.select>
                    ...
                    <x-ui.select name="conta_receber_id" label="Conta a Receber (opcional)" :error="$errors->first('conta_receber_id')">
                        ...
                        <option value="{{ $cr->id }}" @selected(old('conta_receber_id', $nfe->conta_receber_id) == $cr->id)>
                    ...
                    <div class="sm:col-span-2">
                        <x-ui.input
                            ...
                            required
                            value="{{ old('natureza_operacao', $nfe->natureza_operacao) }}"
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <x-ui.input
                        ...
                        value="{{ old('data_emissao', $nfe->data_emissao?->format('Y-m-d')) }}"
                    ...
                    <x-ui.input
                        ...
                        value="{{ old('data_competencia', $nfe->data_competencia?->format('Y-m-d')) }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <x-ui.input
                            ...
                            value="{{ old('valor_produtos', $nfe->valor_produtos) }}"
                    ...
                    <div>
                        <x-ui.input
                            ...
                            x-model="valorFrete"
                            value="{{ old('valor_frete', $nfe->valor_frete) }}"
                    ...
                    <div>
                        <x-ui.input
                            ...
                            value="{{ old('valor_desconto', $nfe->valor_desconto) }}"
                    ...
                    <div>
                        ...
                        <div class="flex items-center rounded-xl ring-1 ring-inset ring-surface-200 dark:ring-surface-700 bg-surface-50 dark:bg-surface-800/50 px-3.5 py-2.5">
                            <span class="text-sm font-semibold text-surface-900 dark:text-white"
                                  x-text="'R$ ' + parseFloat(valorTotal).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })">
                            </span>
                        </div>
            ...
            <x-ui.card class="mb-6">
                ...
                >{{ old('observacoes', $nfe->observacoes) }}</x-ui.textarea>
            </x-ui.card>

            <div class="flex items-center justify-end gap-3">
                <x-ui.button href="{{ route('nfes.show', $nfe) }}" variant="ghost">Cancelar</x-ui.button>
