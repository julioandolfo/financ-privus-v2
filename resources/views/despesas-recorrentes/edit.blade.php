<x-layouts.app title="Editar Despesa Recorrente">
    <div class="max-w-2xl mx-auto">
        ...
        <x-ui.card class="mb-4">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between text-sm">
                    ...
                    <x-ui.badge :variant="match($c->status) { 'pago' => 'success', 'vencido' => 'danger', default => 'secondary' }">
                        {{ ucfirst($c->status) }}
                    </x-ui.badge>
                </div>
                @endforeach
        ...
        <form method="POST" action="{{ route('despesas-recorrentes.update', $recorrencia) }}"
            x-data="{ frequencia: '{{ old('frequencia', $recorrencia->frequencia) }}' }">
            @csrf @method('PUT')
            @include('despesas-recorrentes._form', ['model' => $recorrencia])
            <div class="flex items-center justify-between mt-6">
                <form method="POST" action="{{ route('despesas-recorrentes.destroy', $recorrencia) }}" onsubmit="return confirm('Remover esta recorrência?')">
                    @csrf @method('DELETE')
