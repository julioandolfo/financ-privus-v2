...
<x-ui.card class="mb-4">
    <h2 class="text-sm font-semibold text-surface-700 dark:text-surface-300 mb-4">Dados da Despesa</h2>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <x-ui.select name="fornecedor_id" label="Fornecedor">
            <option value="">— Opcional —</option>
            @foreach($fornecedores as $f)
            <option value="{{ $f->id }}" @selected(old('fornecedor_id', $m?->fornecedor_id) == $f->id)>{{ $f->nome_razao_social }}</option>
            @endforeach
        </x-ui.select>
    </div>
</x-ui.card>
...
<x-ui.card class="mb-4">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <x-ui.select name="status_inicial" label="Status ao Gerar">
            ...
            <option value="pago"     @selected(old('status_inicial', $m?->status_inicial) === 'pago')>Pago automaticamente</option>
        </x-ui.select>
    </div>
