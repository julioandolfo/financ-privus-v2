<x-layouts.app title="Nova Movimentação">
    <div class="max-w-2xl mx-auto">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Nova Movimentação</h1>
                <p class="text-sm text-surface-500 mt-0.5">Registre uma entrada ou saída de caixa</p>
            </div>
        </div>
        @if($errors->any())
        ...
        <form method="POST" action="{{ route('movimentacoes.store') }}">
            ...
            <x-ui.card class="mb-4">
                ...
                {{-- Tipo (entrada/saída) como botões visuais --}}
                <div class="mb-4" x-data="{ tipo: '{{ old('tipo', 'entrada') }}' }">
                    ...
                    <div class="flex gap-3">
                        <label class="flex-1 cursor-pointer" @click="tipo = 'entrada'">
                            <input type="radio" name="tipo" value="entrada" @checked(old('tipo','entrada') === 'entrada') class="sr-only peer">
                            <div class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 text-sm font-medium transition-all
                                peer-checked:border-green-500 peer-checked:bg-green-50 peer-checked:text-green-700
                                dark:peer-checked:border-green-500 dark:peer-checked:bg-green-900/20 dark:peer-checked:text-green-400
                        ...
                        <label class="flex-1 cursor-pointer" @click="tipo = 'saida'">
                            <input type="radio" name="tipo" value="saida" @checked(old('tipo') === 'saida') class="sr-only peer">
                            <div class="flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 text-sm font-medium transition-all
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <x-ui.input name="descricao" label="Descrição" required value="{{ old('descricao') }}" :error="$errors->first('descricao')" />
                    </div>
                    <x-ui.input name="valor" type="number" step="0.01" min="0.01" label="Valor" required placeholder="0,00" value="{{ old('valor') }}" :error="$errors->first('valor')" />
                    <x-ui.input name="data_movimentacao" type="date" label="Data" required value="{{ old('data_movimentacao', today()->format('Y-m-d')) }}" :error="$errors->first('data_movimentacao')" />
                    <x-ui.select name="conta_bancaria_id" label="Conta Bancária" required :error="$errors->first('conta_bancaria_id')">
                        <option value="">— Selecione —</option>
                        @foreach($contas as $c)
                        <option value="{{ $c->id }}" @selected(old('conta_bancaria_id') == $c->id)>{{ $c->nome }} (R$ {{ number_format($c->saldo_atual, 2, ',', '.') }})</option>
                        @endforeach
                    </x-ui.select>
                    <x-ui.input name="data_competencia" type="date" label="Data Competência" value="{{ old('data_competencia') }}" hint="Opcional" />
                </div>
            </x-ui.card>
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <x-ui.select name="forma_pagamento_id" label="Forma de Pagamento">
                        ...
                        <option value="{{ $fp->id }}" @selected(old('forma_pagamento_id') == $fp->id)>{{ $fp->nome }}</option>
                        @endforeach
                    </x-ui.select>
                </div>
            </x-ui.card>
            ...
            <x-ui.card class="mb-6">
                <x-ui.textarea name="observacoes" label="Observações" rows="2" placeholder="Informações adicionais...">{{ old('observacoes') }}</x-ui.textarea>
            </x-ui.card>

            <div class="flex justify-end gap-3">
                <x-ui.button href="{{ route('movimentacoes.index') }}" variant="ghost">Cancelar</x-ui.button>
                <x-ui.button type="submit">Registrar Movimentação</x-ui.button>
            </div>
        </form>
    </div>
</x-layouts.app>
