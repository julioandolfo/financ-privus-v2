<x-layouts.app title="Nova Conta a Pagar">
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl font-semibold text-surface-900 dark:text-white">Nova Conta a Pagar</h1>
                <p class="text-sm text-surface-500 mt-0.5">Registre uma nova obrigação financeira</p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('contas-pagar.store') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="sm:col-span-2">
                        <x-ui.input
                            ...
                            label="Descrição"
                            required
                            placeholder="Ex: Aluguel sede, Energia elétrica..."
                            value="{{ old('descricao') }}"
                    ...
                    <x-ui.select name="fornecedor_id" label="Fornecedor" :error="$errors->first('fornecedor_id')">
                        ...
                        <option value="{{ $f->id }}" @selected(old('fornecedor_id') == $f->id)>{{ $f->nome_razao_social }}</option>
                        @endforeach
                    </x-ui.select>
                    <x-ui.input
            ...
            <x-ui.card class="mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <x-ui.select name="centro_custo_id" label="Centro de Custo" :error="$errors->first('centro_custo_id')">
                        <option value="">— Selecione —</option>
                        @foreach($centrosCusto as $cc)
                        <option value="{{ $cc->id }}" @selected(old('centro_custo_id') == $cc->id)>{{ $cc->nome }}</option>
                        @endforeach
                    </x-ui.select>
                    <x-ui.select name="forma_pagamento_id" label="Forma de Pagamento" :error="$errors->first('forma_pagamento_id')">
                        <option value="">— Selecione —</option>
                        @foreach($formas as $fp)
                        <option value="{{ $fp->id }}" @selected(old('forma_pagamento_id') == $fp->id)>{{ $fp->nome }}</option>
                        @endforeach
                    </x-ui.select>
                    <x-ui.select name="conta_bancaria_id" label="Conta Bancária" :error="$errors->first('conta_bancaria_id')">
                        <option value="">— Selecione —</option>
